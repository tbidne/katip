...
bufferItemTests =
  testGroup
    "bufferItem"
    [ testProperty "0 or negative maximum items always flushes immediately" $
        property $ do
      ...
      testProperty "never buffers too many items" $
        property $ do
          item <- forAllWith showSimplePayloadItem genSimplePayloadItem
          verbosity <- forAll genVerbosity
          ...
          case logAction of
            ...
            _ -> pure (),
      testProperty "never produces too many bytes" $
        property $ do
          item <- forAllWith showSimplePayloadItem genSimplePayloadItem
          verbosity <- forAll genVerbosity
          maxItems <- forAll (Gen.int (Range.linear 0 100))
          ...
          case logAction of
            ...
            _ -> pure (),
      testProperty "never flushes more than 1 item" $
        property $ do
          item <- forAllWith showSimplePayloadItem genSimplePayloadItem
          verbosity <- forAll genVerbosity
          maxItems <- forAll (Gen.int (Range.linear minBound 0))
          ...
          case logAction of
            ...
            _ -> pure (),
      testProperty "flushes when the buffer is exactly at the limit" $
        property $ do
...
renderLineTruncatedTests =
  testGroup
    "renderLineTruncated"
    [ testProperty "always produces an accurate byte count" $
        property $ do
          item <- forAllWith showSimplePayloadItem genSimplePayloadItem
          verbosity <- forAll genVerbosity
          let (builder, Bytes byteCount) = renderLineTruncated' reducedMaxLogLineLength verbosity item
          builderLength builder === byteCount,
      testProperty "always produces a payload smaller than the line limit" $
        property $ do
